 ...
 * checks this and renders nothing, so the day after a festival the site
 * quietly stops shouting instead of counting down into negative numbers.
 *
 * It returns null RIGHT NOW, and by design: `festivals` in site.config.ts is
 * empty since Raksha Bandhan 2026 passed. Add an entry back there and every
 * caller lights up again on its own.
 */
 ...
 * sentence, "Last day to order for {name}", and it was removed on request: we
 * keep taking orders past the cut-off, so the loudest possible version of
 * "you have missed it" is the one line we least want in the announcement bar.
 * Counting down to the day is fine; standing on it and shouting is not.
 */
